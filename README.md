...
| `run src/tools/reset-prep.js` | Pre-reset checklist: sells all stocks, shows installed/pending augs, faction rep & favor, money. Reports only. |
| `run src/tools/reset-prep.js go` | Installs augmentations and resets immediately. |

**Typical end-of-run flow:** `reset-prep.js` (review) → `augmentation-buyer.js install` (buy) → `reset-prep.js go` (install + reset).
